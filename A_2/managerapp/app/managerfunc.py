...
def init_ec2_instances():
    ...
    if not refreshStateandIP(client):  # Fail retirving state form aws. Abandoning operation.
        return False
    ...
    for i in range(8):
        if str(i) not in instances.keys():
            memcacheName = ("ECE1779_A2_Memcache_" + str(i))
...
def getAggregateMissRate1mins(intervals=60):
